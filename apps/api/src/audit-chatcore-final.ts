...
async function auditChatCoreReality() {
  ...
  try {
    ...
    const tables = await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('messages', 'conversations', 'relationships', 'accounts', 'users', 'conversation_participants', 'asset_enrichments') ORDER BY table_name`;
    ...
    const messageColumns = await sql`SELECT column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'messages' ORDER BY ordinal_position`;
    ...
    for (const col of messageColumns) {
      ...
      columnsMap.set(col.column_name as string, col);
    }
    ...
    const relationshipCount = await sql`SELECT COUNT(*) as count FROM relationships`;
    const accountCount = await sql`SELECT COUNT(*) as count FROM accounts`;
    
    console.log(`📊 Estadísticas actuales:`);
    console.log(`   • Messages: ${(messageCount[0] as any).count}`);
    console.log(`   • Conversations: ${(conversationCount[0] as any).count}`);
    console.log(`   • Relationships: ${(relationshipCount[0] as any).count}`);
    console.log(`   • Accounts: ${(accountCount[0] as any).count}`);
    ...
    const orphanMessages = await sql`SELECT m.id, m.sender_account_id, a.username FROM messages m LEFT JOIN accounts a ON m.sender_account_id = a.id WHERE a.id IS NULL LIMIT 10`;
    ...
    const senderIdTypes = await sql`SELECT CASE WHEN sender_account_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN 'UUID' WHEN sender_account_id ~ '^[0-9a-f]{32,}$' THEN 'UUID (sin guiones)' ELSE 'OTHER' END as type, COUNT(*) as count FROM messages GROUP BY type`;
    
    console.log('📊 Tipos de sender_account_id:');
    for (const type of senderIdTypes) {
      console.log(`   • ${(type as any).type}: ${(type as any).count} mensajes`);
    }
    
    // 8. VERIFICAR RELACIONES ROTAS
    console.log('\n=== 8. VERIFICANDO RELACIONES ROTAS ===');
    
    const selfRelationships = await sql`SELECT id, account_a_id, account_b_id FROM relationships WHERE account_a_id = account_b_id`;
    ...
    const emptyMessages = await sql`SELECT id, content FROM messages WHERE (content->>'text' IS NULL OR content->>'text' = '') AND (content->>'media' IS NULL OR jsonb_array_length(COALESCE(content->>'media', '[]'::jsonb)) = 0) AND event_type NOT IN ('reaction', 'system') LIMIT 10`;
