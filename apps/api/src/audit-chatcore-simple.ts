...
async function auditChatCoreReality() {
  ...
  try {
    ...
    const tables = await sql`
      ...
      AND table_name IN ('messages', 'conversations', 'relationships', 'accounts', 'users', 'conversation_participants', 'asset_enrichments')
      ORDER BY table_name
    `;
    ...
    criticalColumns.forEach(({ name, expected, description }) => {
      ...
      if (column) {
        if (column.data_type.includes(expected)) {
          console.log(`✅ ${name}: ${column.data_type} - ${description}`);
        } else {
          console.log(`⚠️  ${name}: ${column.data_type} - ${description} (esperaba ${expected})`);
        }
      } else {
    ...
    const selfRelationships = await sql`
    ...
    if (selfRelationships.length > 0) {
      ...
      selfRelationships.forEach((rel, i) => {
