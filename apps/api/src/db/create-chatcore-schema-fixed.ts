...
async function createChatCoreSchema() {
  ...
  try {
    ...
    // 2. Ejecutar el SQL usando drizzle
    ...
    try {
      ...
      if (messagesColumns.length > 0) {
        const column = messagesColumns[0];
    ...
    try {
      ...
      const softDeleteColumns = await db.execute(`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'messages' 
      ...
      console.log(`   ❌ Error verificando soft delete: ${error}`);
    }
    
    try {
      // Verificar columnas de versionamiento
      const versionColumns = await db.execute(`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'messages' 
      ...
      console.log(`   ❌ Error verificando versionamiento: ${error}`);
    }
    
    try {
      // Verificar columnas de congelamiento
      const freezeColumns = await db.execute(`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
      ...
      for (const col of freezeColumns) {
        console.log(`     ✅ ${col.column_name}`);
      }
    } catch (error) {
      console.log(`   ❌ Error verificando congelamiento: ${error}`);
    }
    ...
    for (const indexName of expectedIndexes) {
      try {
        ...
        console.log(`   ❌ Error verificando ${indexName}: ${error}`);
      }
    }
    ...
    for (const triggerName of expectedTriggers) {
      try {
        ...
        console.log(`   ❌ Error verificando ${triggerName}: ${error}`);
      }
    }
    ...
    for (const viewName of expectedViews) {
      try {
        ...
        console.log(`   ❌ Error verificando ${viewName}: ${error}`);
      }
    }
    
    // 8. Verificar vistas funcionando
    console.log('\n=== 8. VERIFICANDO VISTAS FUNCIONANDO ===');
    
    try {
      const conversationDetails = await db.execute(`SELECT COUNT(*) as count FROM conversation_details LIMIT 1`);
      console.log(`   ✅ conversation_details: FUNCIONA (${conversationDetails[0]?.count} filas)`);
    } catch (error) {
      console.log(`   ❌ conversation_details: ERROR - ${error}`);
    }
    
    try {
      const messageDetails = await db.execute(`SELECT COUNT(*) as count FROM message_details LIMIT 1`);
      console.log(`   ✅ message_details: FUNCIONA (${messageDetails[0]?.count} filas)`);
    } catch (error) {
    ...
    try {
      ...
      const tableCount = finalTables[0]?.count;
