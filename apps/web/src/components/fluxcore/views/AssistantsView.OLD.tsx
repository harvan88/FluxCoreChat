...
export function AssistantsView({ accountId, onOpenTab, assistantId }: AssistantsViewProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'production':
        return <Badge variant="success">Producci??n</Badge>;
      case 'disabled':
  ...
  const handleSelectAssistant = async (summary: Assistant) => {
    // Cargar composici??n completa desde el runtime API
    try {
      ...
      if (res.ok) {
        ...
        // Mapear la composici??n al formato de edici??n
    ...
    // Fallback si falla la carga de composici??n
                ...
                <div className="relative">
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        <option value="">{selectable.length === 0 ? 'No hay m??s bases disponibles' : 'Agregar base de conocimiento...'}</option>
                        {selectable.map((vs) => (
                  ...
                  {(() => {
                    ...
                    return (
                      <select
                        ...
                        disabled={selectable.length === 0}
                      >
                        <option value="">{selectable.length === 0 ? 'No hay m??s herramientas disponibles' : 'Agregar herramienta...'}</option>
                        {selectable.map((tool) => (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-muted text-sm">
                    {selectedAssistant.timingConfig.smartDelay 
                      ...
                      : 'Tiempo de espera antes de responder autom??ticamente'}
                  </span>
                </div>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-subtle text-left">
                ...
                <th className="px-4 py-3 text-xs font-medium text-muted">??ltima modificaci??n</th>
                <th className="px-4 py-3 text-xs font-medium text-muted">Tama??o</th>
