...
|------|------------------|-----------|
| FC-TRANSFORM – FluxCore Identity | ✅ Completado. Core IA renombrado a FluxCore. Cuenta de sistema migrada de @fluxi a @fluxcore. Componentes UI nuevos: CollapsibleSection, SliderInput. | Build verificado |
...
### Estado verificado 2026-01-09 (FluxCore Fase 2)

| Hito | Situación actual | Evidencia |
|------|------------------|-----------|
| FC-AI-UX – FluxCore UX Tabs + CRUD | 🟡 En progreso. Tabs lista/detalle sin duplicados (toggle) + CRUD real sin mocks para bibliotecas (asistentes/instrucciones/vector stores). | `apps/web/src/components/layout/Sidebar.tsx`, `apps/web/src/components/panels/DynamicContainer.tsx` |

#### Hito FC-AI-UX – FluxCore UX Tabs + CRUD (EN PROGRESO 2026-01-09)
...
- Tabs FluxCore con iconos semánticos (Lucide via `TabBar.tsx` + `tab.icon` string key).
- Asistentes: fix payload estricto (evitar `description: null`) + autosave PUT.
- Instrucciones: creación POST + carga por id + guardado PUT.
- Vector Stores: creación POST + carga por id + guardado PUT.

**Pendientes inmediatos:**
- Unificar editor de Instrucciones con el editor del “contexto IA” del perfil (adaptabilidad).
- Definir autosave consistente para Instrucciones y Vector Stores (hoy usan botón Guardar).
- Vector store files: UI real para `/vector-stores/:id/files` (agregar/mostrar/eliminar).

#### Hito FC-TRANSFORM – FluxCore Identity (COMPLETADO 2026-01-08)
**Objetivo:** Transformar Core IA en FluxCore como única extensión preinstalada por defecto.

**Cambios realizados:**
- ✅ Renaming completo: "Core IA" → "FluxCore" en manifest, servicios, frontend
...
**Integración:**
- FluxCorePanel accesible desde extensión core-ai con `view='fluxcore'`
...
## Hito 18: Workspace & Collaborators UI 🟡 PARCIAL (2024-12-09)
...
**Notas de sanitización (gaps detectados):**
- El flujo "Buscar usuario" en `InviteCollaborator` no invita al email real del usuario (usa placeholder `${username}@fluxcore.local`).
...
## Hito 19: Welcome Experience 🟡 PARCIAL (2024-12-09)
...
- [x] Avatar de Fluxi distintivo

**Notas de sanitización (gaps detectados):**
- No se encontró implementación backend para `FC-842 OnboardingConversation`.
- `FirstTimeExperience` existe como componente/hook pero no se encontró integración que lo muestre automáticamente.
...
- ⚠️ `apps/web/public`: existe `pwa-192x192.svg`, pero faltan assets referenciados por el manifest (`pwa-192x192.png`, `pwa-512x512.png`, `favicon.ico`, `apple-touch-icon.png`, `mask-icon.svg`).
- ⚠️ No se encontró implementación explícita de offline fallback UI (FC-903) ni install prompt UI (FC-904).
...
- Modelo de negocio: core-ai gratis, extensiones IA premium monetizan vía Credits
...
- Mantiene `@fluxcore/core-ai` como base gratuita y habilita premium por consumo controlado.
...
| **PC-146** | Config real de core-ai | Alta | AIService: leer configuración desde `extension_installations` (`@fluxcore/core-ai`) (enabled/model/delay/apiKey) | ✅ Completado |
...
| **PC-155** | Consumer IA (integración) | Alta | AIService: si hay sesión activa o puede abrir sesión (consume créditos) → premium; si no → core-ai | ⏳ Pendiente |
...
- [ ] Con saldo = 0 o sesión expirada/agotada: AI cae a `@fluxcore/core-ai` sin romper UX.
...
4. Agotar budget (forzando policy baja) y verificar fallback a `@fluxcore/core-ai`.
...
**Consideraciones:**
- **LLM Integration:** Reutilizar servicio existente de core-ai
...
| **FC-1123** | Conectar AI Suggestions a API real | Media | Remover mock y conectar a core-ai extension | ChatView.tsx |
...
```sql
-- harvan_mj30hgj0 tiene: @fluxcore/website-builder (con Globe), @fluxcore/core-ai
-- karen tiene: @fluxcore/core-ai (sin UI)
```
...
## Hito FC-RUNTIME-3: Migración de CoreIA (Extension Layer)

**Objetivo:** Reemplazar la lógica legacy de `extensions/core-ai` para usar el nuevo servicio FluxCore.

**Duración estimada:** 0.5 semanas

| ID | Tarea | Prioridad | Dependencia | Descripción |
|----|-------|-----------|-------------|-------------|
| **FCR-301** | Actualizar `CoreAIExtension` | ALTA | FCR-203 | Usar `fluxcore.service` en `onMessage` |
...
- [ ] CoreIA opera puramente como interfaz del Runtime
