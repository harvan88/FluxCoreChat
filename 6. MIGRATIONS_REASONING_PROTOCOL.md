...
- **Última verificación completa:** 2026-02-03 00:02 (UTC-03) · Cascade · contenedor `fluxcore-db` levantado vía `docker-compose up -d postgres redis`.
- **Comandos ejecutados:**
  ...
  3. `docker exec fluxcore-db psql -U postgres -d fluxcore -c "\d+ account_deletion_jobs"` → verificó columnas reales, defaults (`status='pending'`, `phase='snapshot'`, `metadata jsonb '{}'`), índices (`pkey`, `account_idx`, `status_idx`) y FKs a `accounts`/`users`.
...
- **Hallazgos vigentes:**
  - Todas las tablas listadas en `packages/db/src/schema/index.ts` aparecen en la DB real (56 tablas, incluye `protected_accounts`, `fluxcore_*`, `credits_*`).
  - `account_deletion_jobs` schema en DB coincide con el ORM y con su uso en `apps/api/src/workers/account-deletion.processor.ts`.
  - Journal de Drizzle no refleja scripts manuales; cualquier cambio adicional debe documentarse aquí.
- **Cambios pendientes de reflejar:**
  - Registrar futuras verificaciones para otras tablas críticas (ej. `messages`, `credits_*`, `fluxcore_vector_stores`).
  - Añadir respaldos (`pg_dump`) antes de la próxima migración real.
