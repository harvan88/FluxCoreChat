...
### Estado verificado 2026-01-25 (Alineación OpenAI)

| Hito | Situación actual | Evidencia |
|------|------------------|-----------|
| FC-OPENAI-MIRROR – OpenAI Vector Stores Mirroring | ✅ Completado. Espejo 1:1 con OpenAI implementado. Auto-save, resolución de nombres reales, políticas de expiración simplificadas y ecosistema de asistentes vinculados. | Build verificado 2026-01-25 |

### Estado verificado 2026-01-27 (Account Deletion Agent)

| Hito | Situación actual | Evidencia |
|------|------------------|-----------|
| AD-100 – Guardianes y Prohibiciones Absolutas | ✅ Completado. `protected_accounts` catalogado vía `seed:protected-accounts`, guard reutilizable (`ensureAccountDeletionAllowed`) integrado en worker y pruebas unitarias Vitest. | `packages/db/src/seed-protected-accounts.ts`, `apps/api/src/services/account-deletion.guard.ts`, `apps/api/src/workers/account-deletion.worker.ts`, `apps/api/src/services/account-deletion.guard.test.ts` |
| AD-110 – Autorización Formal | 🟡 En progreso. Falta middleware `requireAccountDeletionAuth` integrado en rutas `/accounts/:id/delete/*`, verificación automática de `ACCOUNT_DELETE_FORCE` en seeds/UI y pruebas de integración (403/401). | `apps/api/src/routes/accounts.routes.ts`, `apps/api/src/middleware/account-deletion-auth.ts`, `packages/db/src/seed-system-admins.ts` |
| AD-120 – Snapshot & Portal Post-confirmación | ✅ Actualizado 2026-01-30. Snapshot/token se generan al confirmar, el worker bloquea avance hasta `snapshotReadyAt` y se habilitó portal público `/account-deletions/:jobId?token=…` con logs de actividad. FK cascada removida para conservar jobs/logs tras el purge y se retiró el `AccountDeletionBanner` en favor de redirección inmediata + toasts. | `apps/api/src/services/account-deletion.service.ts`, `apps/api/src/workers/account-deletion.worker.ts`, `apps/api/src/services/account-deletion.external.ts`, `apps/web/src/pages/AccountDeletionPortalPage.tsx`, `apps/web/src/store/uiStore.ts`, `apps/web/src/components/layout/Layout.tsx`, `packages/db/migrations/028-030_*` |
...
#### Subhito H3-TM-BE: Template CRUD Backend 🟡 En progreso

**Objetivo:** Proveer persistencia real de plantillas por cuenta (`accountId`) con CRUD mínimo y reutilizar los assets existentes sin tocar ChatCore/ExtensionHost.

**Alcance:**
- REST `/templates` (GET listado, POST crear, PUT actualizar, DELETE eliminar).
- Payload declarativo: nombre, contenido, variables, tags, referencias de assets (`assetRefs`).
- Reutilizar `/templates/:id/assets` de asset-relations (sin lógica duplicada).
- El envío de mensajes sigue pasando por `POST /messages`; no se agrega metadata de plantilla.

| ID | Tarea | Prioridad | Estado |
|----|-------|-----------|--------|
| TM-BE-201 | Schema + repo + servicio TemplateService (Drizzle) | Alta | ✅ |
| TM-BE-202 | Endpoints `GET/POST /templates` | Alta | ✅ |
| TM-BE-203 | Endpoints `PUT/DELETE /templates/:id` + validaciones scope account | Alta | ✅ |
| TM-BE-204 | Tests (unit + integration API) | Media | 🟡 |
| TM-BE-205 | Documentar payload y ejemplo de uso | Media | ⬜ |

**Checklist de aceptación:**
- [x] CRUD completo responde 2xx/4xx coherentes y falla fuera del scope del account.
- [x] No se modifica MessageCore ni ExtensionHost.
- [x] `/templates/:id/assets` se usa tal cual para adjuntar archivos.
- [ ] Autosave en UI puede persistir usando este CRUD sin mocks (depende de integración UI).

#### Subhito H3-TM: UI – Template Manager 🔴 BLOQUEADO (depende TM-BE)

**Objetivo:** Entregar el sistema de plantillas como feature 100% UI usando el CRUD existente (ToolsSidebar → TemplateManager → TemplateEditor).

**Duración estimada:** 1 semana · **Dependencias:** ViewRegistry y Component Library (H3/H13) · **Alcance congelado:** sin cambios en backend/ChatCore.

| ID | Tarea | Prioridad | Estado |
|----|-------|-----------|--------|
| TM-101 | Crear estructura `apps/web/src/components/templates/` (types, hook, index) | Media | ⬜ |
| TM-102 | Implementar `TemplateManager` (sidebar view + filtros + abrir tabs) | Alta | ⬜ |
| TM-103 | Implementar `TemplateEditor` + `TemplatePreview` con detección de variables | Alta | ⬜ |
| TM-104 | Integrar `TemplateAssetPicker` y vínculo con assets existentes | Media | ⬜ |
| TM-105 | Registrar vistas en ViewRegistry y ToolsSidebar (tab `template-panel`, tab `template-editor`) | Alta | ⬜ |
| TM-106 | QA UI: estados loading/empty/error, responsive y accesibilidad básica | Media | ⬜ |

**Checklist de aceptación:**
- [ ] Crear/editar/duplicar/eliminar plantilla desde ToolsSidebar usando componentes canónicos.
- [ ] Variables detectadas ({{variable}}) mostradas en editor y preview en vivo.
- [ ] Assets adjuntos vía picker reaprovechando el flujo existente (sin endpoints nuevos).
- [ ] Abrir/cerrar editor como tab (`template-editor`) sin duplicados; panel principal (`template-panel`) se administra desde Tools.
- [ ] No se tocan archivos de backend/ChatCore/ExtensionHost.
...
### Hito PC-3: Automation Triggers Avanzados ✅ COMPLETADO (Revisión 2026-02-02)
...
| ID | Tarea | Prioridad | Descripción | Estado |
|----|-------|-----------|-------------|--------|
| **PC-120** | Schedule trigger | Alta | Persistencia y ejecución con Croner, soporte timezone | ✅ apps/api/src/services/automation-controller.service.ts · automation-scheduler.service.ts |
| **PC-121** | Webhook trigger | Alta | Ingesta segura de webhooks externos con tokens y validaciones | ✅ apps/api/src/routes/automation.routes.ts · automation-controller.service.ts |
| **PC-122** | Trigger UI | Media | Configurar triggers en frontend | ✅ apps/web/src/hooks/useAutomation.ts · components/settings/AutomationSection.tsx |
...
**Criterios de éxito:**
- [x] Reglas ejecutan workflows en horarios programados con timezone configurable (Croner + timezone metadata, ver automation-scheduler.service.ts).
- [x] Webhooks externos pueden disparar reglas (`POST /automation/webhook/:token` validando tokens y ejecutando workflows).
...
**Prioridad:** CRÍTICA (errores críticos en producción)

**Referencia:** Auditoría de código - 19 errores encontrados

| ID | Tarea | Prioridad | Verificación | Estado |
|----|-------|-----------|--------------|--------|
...
3. Implementar políticas de expiración compatibles
4. Unificar lógica de eliminación local/remota

**Tareas**:
- VS-ALIGN-110: Servicio unificado de eliminación con limpieza en cascada
- VS-ALIGN-111: Capa de conversión de políticas de expiración (días ↔ duración)
- VS-ALIGN-112: Mecanismo de sincronización de estado
- VS-ALIGN-113: UI de colección de archivos en VectorStoreView
