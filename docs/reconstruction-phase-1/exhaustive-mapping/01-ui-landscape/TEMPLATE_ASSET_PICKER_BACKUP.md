---
id: "template-asset-picker-backup"
...
# TemplateAssetPicker - Gestión de Archivos Adjuntos (BACKUP)
...
```typescript
// apps/web/src/components/templates/TemplateAssetPicker.tsx
export default function TemplateAssetPicker({ 
  accountId, 
  templateId, 
  onAssetsChange,
  ...
  const { assets: fetchedAssets, loading: assetsLoading } = useTemplateAssets(templateId, accountId);
  const { uploadAsset, linkAsset, unlinkAsset, deleteAsset } = useAssetActions();
  
  // Efectos
  useEffect(() => {
    setAssets(fetchedAssets || []);
    setLoading(assetsLoading);
  }, [fetchedAssets, assetsLoading]);
  ...
  // Renderizado
  return (
    <div className="template-asset-picker">
      <Header />
      <UploadZone />
      <AssetList />
      <EmptyState />
...
const updateUploadState = useCallback((updates: Partial<typeof uploadState>) => {
...
const handleMultipleUpload = async (files: File[]) => {
  ...
  if (failed.length > 0) {
    setError(`${failed.length} archivos fallaron al subir`);
  }
...
```typescript
const AssetPreview = ({ asset, isOpen, onClose }) => {
  const [previewUrl, setPreviewUrl] = useState<string>('');
  const [loading, setLoading] = useState(false);
  
  // Generar preview para diferentes tipos
  const generatePreview = useCallback(async () => {
    ...
    // Para otros tipos, generar thumbnail
    setLoading(true);
    try {
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="asset-preview">
        ...
        {loading ? (
          ...
          <PreviewContent url={previewUrl} mimeType={asset.mimeType} />
        )}
...
- **Event listeners:** 5-8 activos
- **State updates:** 2-3 por operación
- **API calls:** 1 inicial + 1 por upload

### Performance Targets
- **Upload speed:** > 1MB/s para imágenes
- **Preview generation:** < 500ms
- **Render time:** < 16ms para 20 items
- **Memory usage:** < 40MB para 100 assets
...
2. Cargar assets existentes
3. Configurar event listeners

### Update
1. Actualizar estado con nuevos uploads
2. Actualizar progreso de upload
3. Actualizar lista de assets

### Unmount
1. Cancelar uploads pendientes
2. Limpiar event listeners
3. Limpiar URLs de blobs
...
### Responsive Design
- **Mobile:** Layout vertical, scroll optimizado
- **Tablet:** Grid 2 columnas
- **Desktop:** Grid 3 columnas
- **Large Desktop:** Grid 4 columnas
...
```typescript
// POST /assets/upload
const uploadAsset = async (file: File, accountId: string, options?: UploadOptions): Promise<Asset> => {
...
// POST /templates/{templateId}/assets/{assetId}
const linkAsset = async (accountId: string, templateId: string, assetId: string, slot: string): Promise<void> => {
  await api.post(`/templates/${templateId}/assets/${assetId}`, {
    accountId,
    slot
  });
};

// DELETE /templates/{templateId}/assets/{assetId}
const unlinkAsset = async (accountId: string, templateId: string, assetId: string): Promise<void> => {
...
const deleteAsset = async (assetId: string, accountId: string): Promise<void> => {
...
### Error Handling
```typescript
const handleUploadError = (error: AxiosError, file: File) => {
  ...
  if (error.response?.status === 413) {
    ...
    showFileTypeError(file.type);
  } else if (error.response?.status === 422) {
    // Error de validación
    showValidationError(error.response.data.message);
  } else {
    // Error genérico
    showGenericUploadError(error.message);
  }
...
describe('TemplateAssetPicker', () => {
  it('should render upload zone', () => {
    render(<TemplateAssetPicker accountId="test" templateId="test" />);
...
### Integration Tests
```typescript
describe('TemplateAssetPicker Integration', () => {
  it('should upload and link asset', async () => {
    const mockUpload = jest.fn().mockResolvedValue({ assetId: 'asset-1' });
    const mockLink = jest.fn().mockResolvedValue({});
...
```typescript
// Upload de archivo
analytics.track('asset_upload_start', {
  accountId,
  templateId,
  fileName: file.name,
  fileSize: file.size,
  fileType: file.type,
  uploadMethod: 'drag_drop' // o 'file_selector'
});

// Upload completado
analytics.track('asset_upload_complete', {
  accountId,
  templateId,
...
// Preview de asset
analytics.track('asset_preview', {
  accountId,
  templateId,
  assetId: asset.assetId,
  assetType: asset.mimeType,
  previewDuration: previewEndTime - previewStartTime
});
```
...
analytics.track('asset_upload_time', {
  uploadTime,
  fileSize: file.size,
  uploadSpeed: file.size / (uploadTime / 1000), // bytes/sec
  fileType: file.type
});

// Tiempo de preview
const previewTime = performance.now() - previewStartTime;
analytics.track('asset_preview_time', {
  previewTime,
  assetType: asset.mimeType,
...
1. **Batch upload:** Subida múltiple simultánea
2. **Drag & drop mejorado:** Feedback visual mejorado
3. **File validation:** Validación más estricta
4. **Upload retry:** Reintentos automáticos

### Medium Term (1-2 meses)
1. **Image editing:** Recorte y edición básica
2. **Video support:** Soporte para videos
3. **Cloud storage:** Integración con cloud storage
4. **Asset analytics:** Estadísticas de uso

### Long Term (3-6 meses)
1. **AI-powered tagging:** Tags automáticos con IA
2. **Asset marketplace:** Marketplace de assets
3. **Version control:** Control de versiones de assets
4. **Collaborative assets:** Assets compartidos entre usuarios
...
- **`useTemplateAssets.ts`**: Hook para obtener assets
- **`useAssetActions.ts`**: Hook para acciones de assets

### Servicios Relacionados
- **`asset.service.ts`**: Servicio de backend para assets
