...
export interface AISuggestion {
  ...
  toolUse?: AITraceEntry['toolUse'];
  toolsUsed?: AIToolExecutionRecord[];
  promptDebug?: {
    builtPrompt: {
      systemPrompt: string;
      messages: Array<{ role: string; content: string | null }>;
      messagesWithCurrent: Array<{ role: string; content: string | null }>;
    };
...
export interface RuntimeServices {
  resolveActiveAssistant: (accountId: string) => Promise<AssistantComposition | null>;
  ...
  sendTemplate: (params: { accountId: string; conversationId: string; templateId: string; variables?: Record<string, string>; }) => Promise<{ messageId?: string; status?: string } | null>;
}
...
export class FluxCoreExtension {
  ...
  /**
   * Inject runtime services for direct in-process calls (replaces HTTP self-calls).
   */
  setRuntimeServices(services: RuntimeServices): void {
    this.runtimeServices = services;
  }

  /**
   * Fetch active assistant — uses injected service or HTTP fallback.
   */
  private async fetchActiveAssistant(accountId: string): Promise<AssistantComposition | null> {
    if (this.runtimeServices) {
      return this.runtimeServices.resolveActiveAssistant(accountId);
    }

    // HTTP fallback (legacy — only if services not injected)
    try {
  ...
  private async listAuthorizedTemplates(accountId: string): Promise<Array<{ id: string; name?: string; category?: string; variables?: string[]; instructions?: string | null }>> {
    if (this.runtimeServices) {
      return this.runtimeServices.listTemplates(accountId);
    }
  ...
  private async sendTemplateTool(params: {
    ...
    if (this.runtimeServices) {
      return this.runtimeServices.sendTemplate(params);
    }
  ...
  private async fetchRAGContext(
    ...
    if (this.runtimeServices) {
      return this.runtimeServices.fetchRagContext(accountId, query, vectorStoreIds);
    }
  ...
  async generateSuggestion(
    ...
    // Canon v7.0: Inject PolicyContext
    if (policyContext) {
    ...
    try {
      let providerOrder = this.getProviderOrder(cfg);
      ...
      const trace: AITraceEntry = {
        ...
        mode: cfg.mode === 'auto' ? 'auto' : 'suggest',
        model: cfg.model,
      ...
      trace.toolUse = {
        ...
        toolsCalled: toolUseCalls.map(call => call.name),
      ...
      // WES-170: Detectar si se llamó a la herramienta 'propose_work'
      let proposedWork: AISuggestion['proposedWork'] | undefined;
      const proposeWorkCall = toolUseCalls.find(tc => tc.name === 'propose_work');

      if (proposeWorkCall && proposeWorkCall.args) {
        proposedWork = {
          workDefinitionId: proposeWorkCall.args.workDefinitionId,
          intent: proposeWorkCall.args.intent,
          candidateSlots: proposeWorkCall.args.candidateSlots || [],
          confidence: 0.9, // Default confidence for explicit tool call
          traceId,
        };
        console.log(`[fluxcore] 📦 Work Proposed via tool: ${proposedWork.workDefinitionId}`);
      }
      ...
      const suggestion: AISuggestion = {
        ...
        toolUse: trace.toolUse,
        toolsUsed: trace.toolsUsed,
        proposedWork, // Inyectar propuesta de trabajo
        promptDebug: {
          builtPrompt: {
            systemPrompt: prompt.systemPrompt,
            messages: prompt.messages,
  ...
  deleteTrace(params: { accountId: string; traceId: string }): boolean {
    const index = traces.findIndex((t) => t.id === params.traceId);
    if (index === -1) return false;
    if (traces[index].accountId !== params.accountId) return false;
  ...
  /**
   * Public LLM call for Agent Runtime Engine (Fase 3).
   * Lightweight wrapper around createChatCompletionWithFallback.
   */
  async callLLM(params: {
    ...
    const providerOrder = params.providerOrder || this.getProviderOrder();
    const messages: OpenAIChatMessage[] = params.messages.map(m => ({
      role: m.role,
      content: m.content,
    }));

    // Create a minimal trace object for createChatCompletionWithFallback
    const trace: AITraceEntry = {
      id: crypto.randomUUID(),
      createdAt: new Date().toISOString(),
      accountId: 'system',
      conversationId: 'raw-completion',
      messageId: 'raw',
      mode: 'suggest',
      model: params.model,
    ...
    const result = await this.createChatCompletionWithFallback({
      ...
      maxTokensOverride: params.maxTokens || 256,
  ...
  private getProviderOrder(cfgSource?: FluxCoreConfig): Array<{ provider: 'groq' | 'openai'; baseUrl: string; apiKey: string; keySource?: string }> {
    const src = cfgSource || this.config;
    console.log('[fluxcore] getProviderOrder - config.providerOrder length:', src.providerOrder?.length || 0);
    ...
    if (Array.isArray(src.providerOrder) && src.providerOrder.length > 0) {
      console.log('[fluxcore] Usando providerOrder de config:', src.providerOrder.map(p => p.provider));
      return src.providerOrder.filter((p) => typeof p?.apiKey === 'string' && p.apiKey.length > 0);
    }
    ...
    const groqKey = src.apiKey || process.env.GROQ_API_KEY;
    if (groqKey) {
      providers.push({
        provider: 'groq',
  ...
  private async createChatCompletionWithFallback(params: {
    ...
    const cfgSrc = params.config || this.config;
    const timeoutMs = cfgSrc.timeoutMs || 15000;
    const maxAttemptsPerProvider = 2;

    // Modelo base solicitado (puede ser de cualquier provider)
    const requestedModel = params.modelOverride || cfgSrc.model;
    const maxTokens = params.maxTokensOverride ?? cfgSrc.maxTokens;
    const temperature = params.temperatureOverride ?? cfgSrc.temperature;
